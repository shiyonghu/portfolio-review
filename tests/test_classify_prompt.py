...
def test_y_accepts_suggestion() -> None:
    ...
    action, bucket = prompt_confirmed_bucket(
        asset_name="FOO",
        suggestion=BucketSuggestion(suggested_bucket="Equity", reason="ETF", error=None),
...
def test_y_without_suggestion_reprompts_then_m() -> None:
    ...
    action, bucket = prompt_confirmed_bucket(
        asset_name="BAR",
        suggestion=BucketSuggestion(suggested_bucket=None, reason="", error="offline"),
        read_line=lambda _p: lines.popleft(),
...
def test_n_skips() -> None:
    action, bucket = prompt_confirmed_bucket(
        asset_name="X",
        suggestion=BucketSuggestion(suggested_bucket="Gold", reason="x", error=None),
...
def test_m_invalid_then_valid() -> None:
    ...
    action, bucket = prompt_confirmed_bucket(
        asset_name="Z",
        suggestion=BucketSuggestion(suggested_bucket=None, reason="", error=None),
        read_line=lambda _p: lines.popleft(),
        write=lambda _s: None,
    )
    assert action == "persist"
